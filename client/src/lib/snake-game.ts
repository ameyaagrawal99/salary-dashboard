...
export type DeathReason = "wall" | "self" | "win" | null;

export type StepEvent = "idle" | "move" | "eat" | "game-over" | "win";
...
export function stepGame(
  ...
  if (isOutOfBounds(candidateHead, state.gridSize)) {
    return {
      ...state,
      direction,
      gameOver: true,
  ...
  if (isOnSnake(candidateHead, snakeForCollision)) {
    return {
      ...state,
      direction,
      gameOver: true,
  ...
  return {
    ...
    deathReason: noFoodSlotLeft ? "win" : null,
    lastEvent: noFoodSlotLeft ? "win" : "eat",
  };
}
